...
download_scanner() {
    ...
    if ! curl -sSL https://raw.githubusercontent.com/otto-de/OreNPMGuard/main/shai_hulud_scanner.py -o "$TEMP_DIR/shai_hulud_scanner.py"; then
        log_error "Failed to download Python scanner"
        return 1
    fi

    if ! curl -sSL https://raw.githubusercontent.com/otto-de/OreNPMGuard/main/affected_packages.yaml -o "$TEMP_DIR/affected_packages.yaml"; then
